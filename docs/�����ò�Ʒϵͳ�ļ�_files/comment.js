$(document).ready(function () {
    ...
    function quoteComment(commentId, list) {
		...
		editor.parseHtml("[quote=" + (comment.UserName == null ? "游客" : comment.UserName) + "]" + content + "[/quote]\r\n");
    }
    //回复评论
    function replyComment(commentId, list) {
        var comment = getComment(commentId, list);
        editor.parseHtml('[reply]' + comment.UserName + "[/reply]\r\n");
    ...
    $("#ubbtools").children().click(function () {
        ...
        switch (code) {
            /*case "indent":
            editor.parseHtml("\t" + selectedValue);
            break;
            case "url":
            var title = prompt("链接标题：","");
            var link = prompt("链接地址：", "http://");
            editor.parseHtml("[url=" + link + "]" + title + "[/url]");
            break;*/ 
            case "code":
                ...
                lang_list.children().each(function () {
                    $(this).unbind("click").click(function () {
                        editor.parseHtml("[code=" + $.trim(this.href.split('#')[1]) + "]\n" + selectedValue + "\n[/code]");
                        lang_list.hide();
                    });
                });
                ...
                break;
            default:
                editor.parseHtml("[" + code + "]" + selectedValue + "[/" + code + "]");
                break;
        }
        return false;
    });
